...
public class SessionSearchTrees {
	...
	private static Stream<String> getTooltipLines(Stream<ItemStack> stream, TooltipContext tooltipContext, TooltipFlag tooltipFlag) {
		return stream.<Component>flatMap(itemStack -> itemStack.getTooltipLines(tooltipContext, null, tooltipFlag).stream())
			...
			() -> {
				...
				this.creativeByNameSearch = CompletableFuture.supplyAsync(
					() -> new FullTextSearchTree<>(
						...
						itemStack -> itemStack.getItemHolder().unwrapKey().<Identifier>map(ResourceKey::identifier).stream(),
						list
					),
					Util.backgroundExecutor()
				);
