...
public class QuadrupedModel<T extends LivingEntityRenderState> extends EntityModel<T> {
	...
	static void createLegs(PartDefinition partDefinition, boolean bl, boolean bl2, int i, CubeDeformation cubeDeformation) {
		CubeListBuilder cubeListBuilder = CubeListBuilder.create().mirror(bl2).texOffs(0, 16).addBox(-2.0F, 0.0F, -2.0F, 4.0F, i, 4.0F, cubeDeformation);
		CubeListBuilder cubeListBuilder2 = CubeListBuilder.create().mirror(bl).texOffs(0, 16).addBox(-2.0F, 0.0F, -2.0F, 4.0F, i, 4.0F, cubeDeformation);
