...
public record TelemetryProperty<T>(String id, String exportKey, Codec<T> codec, TelemetryProperty.Exporter<T> exporter) {
	...
	public static TelemetryProperty<LongList> longSamples(String string, String string2) {
		return create(
			...
			(telemetryPropertyContainer, stringx, longList) -> telemetryPropertyContainer.addProperty(
				stringx, longList.longStream().mapToObj(String::valueOf).collect(Collectors.joining(";"))
			)
		);
	}
	...
	public enum GameMode implements StringRepresentable {
		...
		GameMode(final String string2, final int j) {
			this.key = string2;
			this.id = j;
		}
	...
	public enum ServerType implements StringRepresentable {
		...
		ServerType(final String string2) {
			this.key = string2;
		}
