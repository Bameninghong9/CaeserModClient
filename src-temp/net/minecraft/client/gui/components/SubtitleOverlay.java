...
public class SubtitleOverlay implements SoundEventListener {
	...
	static class Subtitle {
		...
		public SubtitleOverlay.@Nullable SoundPlayedAt getClosest(Vec3 vec3) {
			if (this.playedAt.isEmpty()) {
				return null;
			} else {
				return this.playedAt.size() == 1
					? this.playedAt.getFirst()
					: this.playedAt.stream().min(Comparator.comparingDouble(soundPlayedAt -> soundPlayedAt.location().distanceTo(vec3))).orElse(null);
			}
		}
