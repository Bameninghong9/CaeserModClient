...
public class CreativeModeInventoryScreen extends AbstractContainerScreen<CreativeModeInventoryScreen.ItemPickerMenu> {
			...
			if (slot == null && selectedTab.getType() != Type.INVENTORY && clickType != ClickType.QUICK_CRAFT) {
				...
				if (slot == this.destroyItemSlot && bl) {
					...
					if (AbstractContainerMenu.getQuickcraftHeader(j) == 2) {
						...
						if (clickType == ClickType.THROW && !itemStack.isEmpty() && !this.menu.getCarried().isEmpty()) {
	...
	private void updateVisibleTags(String string) {
		...
		BuiltInRegistries.ITEM.getTags().<TagKey>map(Named::key).filter(tagKey -> predicate.test(tagKey.location())).forEach(this.visibleTags::add);
	}
