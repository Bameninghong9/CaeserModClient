	...
	private static void openCreateWorldScreen(
		...
		CompletableFuture<WorldCreationContext> completableFuture = WorldLoader.load(
			...
			dataLoadContext -> new DataLoadOutput(
				new DataPackReloadCookie(function.apply(dataLoadContext), dataLoadContext.dataConfiguration()), dataLoadContext.datapackDimensions()
			),
	...
	private boolean createNewWorld(LayeredRegistryAccess<RegistryLayer> layeredRegistryAccess, WorldData worldData) {
		...
		if (optional.isEmpty()) {
			...
			this.minecraft
				...
				.createLevelFromExistingSettings(optional.get(), worldCreationContext.dataPackResources(), layeredRegistryAccess, worldData);
			return true;
		}
	}
	...
	private void applyNewPackConfig(PackRepository packRepository, WorldDataConfiguration worldDataConfiguration, Consumer<WorldDataConfiguration> consumer) {
		...
		WorldLoader.load(
			...
			.thenApply(worldCreationContext -> {
				...
				return (WorldCreationContext)worldCreationContext;
			})
