...
public class ModelManager implements PreparableReloadListener {
	...
	public final CompletableFuture<Void> reload(SharedState sharedState, Executor executor, PreparationBarrier preparationBarrier, Executor executor2) {
		...
		return CompletableFuture.allOf(
			...
			.<ModelManager.ReloadState>thenCompose(preparationBarrier::wait)
	...
	private static CompletableFuture<Map<Identifier, UnbakedModel>> loadBlockModels(ResourceManager resourceManager, Executor executor) {
		return CompletableFuture.<Map>supplyAsync(() -> MODEL_LISTER.listMatchingResources(resourceManager), executor)
			.thenCompose(
				map -> {
