...
public class EntityRenderers {
	...
	public static Map<EntityType<?>, EntityRenderer<?, ?>> createEntityRenderers(EntityRendererProvider.Context context) {
		...
		PROVIDERS.forEach((entityType, entityRendererProvider) -> {
			try {
				builder.put((EntityType<?>)entityType, entityRendererProvider.create(context));
			} catch (Exception exception) {
				throw new IllegalArgumentException("Failed to create model for " + BuiltInRegistries.ENTITY_TYPE.getKey(entityType), exception);
			}
		});
