...
public class FrameGraphBuilder {
	...
	<T> FrameGraphBuilder.InternalVirtualResource<T> createInternalResource(
		String string, ResourceDescriptor<T> resourceDescriptor, FrameGraphBuilder.@Nullable Pass pass
	) {
	...
	private void discoverAllRequiredPasses(FrameGraphBuilder.Pass pass, BitSet bitSet, Deque<FrameGraphBuilder.Pass> deque) {
		...
		while (!deque.isEmpty()) {
			FrameGraphBuilder.Pass pass2 = deque.poll();
	...
	private void resolvePassOrder(FrameGraphBuilder.Pass pass, BitSet bitSet, BitSet bitSet2, List<FrameGraphBuilder.Pass> list) {
		if (bitSet2.get(pass.id)) {
			String string = bitSet2.stream().mapToObj(ix -> this.passes.get(ix).name).collect(Collectors.joining(", "));
		...
		if (bitSet.get(pass.id)) {
			...
			for (int i = pass.requiredPassIds.nextSetBit(0); i >= 0; i = pass.requiredPassIds.nextSetBit(i + 1)) {
				this.resolvePassOrder(this.passes.get(i), bitSet, bitSet2, list);
			}
	...
	static class ExternalResource<T> extends FrameGraphBuilder.VirtualResource<T> {
		...
		public ExternalResource(String string, FrameGraphBuilder.@Nullable Pass pass, T object) {
			super(string, pass);
	...
	static class Handle<T> implements ResourceHandle<T> {
		...
		Handle(FrameGraphBuilder.VirtualResource<T> virtualResource, int i, FrameGraphBuilder.@Nullable Pass pass) {
	...
	static class InternalVirtualResource<T> extends FrameGraphBuilder.VirtualResource<T> {
		...
		public InternalVirtualResource(int i, String string, FrameGraphBuilder.@Nullable Pass pass, ResourceDescriptor<T> resourceDescriptor) {
			super(string, pass);
			this.id = i;
	...
	abstract static class VirtualResource<T> {
		...
		public VirtualResource(String string, FrameGraphBuilder.@Nullable Pass pass) {
			this.name = string;
